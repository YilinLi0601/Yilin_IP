...
EMA_select %>%
  ...
  ungroup() %>%
  summarise(min_com = min(compliance),
            max_com = max(compliance))
...
EMA_final <- EMA_SIonly %>%
  select(ID, Group, survey_dtime, current_affect, passiveSI, activeSI, SMI, SI_any, stress_any, stress_int, pos_any, pos_int, stressor_stamp, pos_stamp, post_stress_affect, post_pos_affect)
saveRDS(EMA_final, "EMA_final.rds")
```

## Demographics Calc
```{r}
# Read data
demo <- read.csv("DEETs2data_Baseline_cleaned 082425.csv")
glimpse(demo)
# Filtering N=59 used in main analysis
IDs <- EMA_mplus %>%
  distinct(ID) %>%
  pull(ID)
demo59 <- demo %>%
  filter(BL_C_ID %in% IDs)
# Gender
demo59 %>%
  count(BL_C_Sex)
...
ethnicity <- demo59 %>%
  summarise(
    white = sum(BL_C_Race_White, na.rm = TRUE),
    asian = sum(BL_C_Race_Asian, na.rm = TRUE),
    black = sum(BL_C_Race_Black, na.rm = TRUE),
    other = sum(BL_C_Race_Other, na.rm = TRUE)
  ) %>%
...
#Age
demo59 %>%
  summarise(mean_age = mean(BL_C_Age),
            sd_age = sd(BL_C_Age))
```
